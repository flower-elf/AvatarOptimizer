...
## [Unreleased]
### Added
- Bug Report Helper now includes `TraceAndOptimizeState` in the `AtTheBeginning` phase as a pretty-printed JSON file [`#PRNUM`](https://github.com/anatawa12/AvatarOptimizer/pull/PRNUM)
...
- Ignore Transform is incorrectly applied for root transform [`#1721`](https://github.com/anatawa12/AvatarOptimizer/pull/1721)
  - This includes ignore transform generated by "ignore other physbone" so this bug will affect avatars with multiple physbones at same target bone.
- MergePhysBone Collider may merge differently toggled PhysBoneColliders when colliders are targeting external bones [`#1723`](https://github.com/anatawa12/AvatarOptimizer/pull/1723)
  - The merge process was checking IsActive animation only on the target bone's hierarchy, so two colliders targeting the same external bone but controlled by different toggle animations were incorrectly merged into one.
